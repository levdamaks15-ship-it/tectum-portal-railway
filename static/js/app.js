...
// Global handler: prevent typing '-' (minus) in numeric inputs
document.addEventListener('keydown', function (e) {
    if (e.target && (e.target.type === 'number' || e.target.classList.contains('numeric-input'))) {
        // Disallow minus key (both main keyboard and numpad)
...
// Global handler: sanitise pasted or entered negative numbers
...
// Universal Click-to-Open Picker for all Date, Month and Time inputs across the portal
document.addEventListener('click', function (e) {
    const input = e.target.closest('input');
    if (!input) return;

    // 1. If element has Flatpickr attached or is altInput
    if (input._flatpickr) {
    ...
    // If it's the altInput generated by flatpickr
    if (input.classList.contains('flatpickr-input')) {
        const fp = input._flatpickr || (input.nextElementSibling && input.nextElementSibling._flatpickr) || (input.previousElementSibling && input.previousElementSibling._flatpickr);
        if (fp && !fp.isOpen) {
            fp.open();
            return;
        }
    ...
    // 2. Native HTML5 Date, Month or Time inputs (only if NOT handled by Flatpickr)
    const pickerTypes = ['date', 'month', 'time', 'datetime-local', 'week'];
    if (pickerTypes.includes(input.type) && !input.classList.contains('flatpickr-input')) {
        if (typeof input.showPicker === 'function') {
            try {
                input.showPicker();
            } catch (err) {
                // Ignore AbortError / SecurityError if already open or unsupported
            }
        }
    }
});
...
function setupTimePickers() {
    ...
    const monthConfigs = [
        {
            id: 'crew-plans-month',
            onChange: () => { if (typeof loadCrewPlansFulfillment === 'function') loadCrewPlansFulfillment(); }
        },
        {
            id: 'summary-filter-month',
            onChange: () => { if (typeof onSummaryMonthChange === 'function') onSummaryMonthChange(); }
        },
        {
            id: 'daily-report-month',
            onChange: () => { if (typeof onDailyReportMonthChange === 'function') onDailyReportMonthChange(); }
        }
    ];
    ...
    monthConfigs.forEach(cfg => {
        ...
        if (el) {
            ...
            const fpOptions = {
                ...
                dateFormat: "Y-m",
                altInput: true,
                altFormat: "F Y", // e.g. "Сентябрь 2026"
                allowInput: true,
                onChange: function(selectedDates, dateStr, instance) {
                    if (cfg.onChange) cfg.onChange(dateStr);
                }
            };
...
// Custom Bento Select Component (Flatpickr Dropdown Style Parity)
function initCustomSelect(selectEl) {
    if (!selectEl || selectEl.dataset.customSelectInitialized === 'true') return;
    // Do not transform multiple or hidden selects
    if (selectEl.multiple || selectEl.type === 'hidden') return;
    ...
    wrapper.appendChild(selectEl); // Keep select inside wrapper for tidy DOM
}

function initAllCustomSelects(root = document) {
    const selects = root.querySelectorAll('select:not([multiple]):not(.no-custom-select)');
    selects.forEach(s => initCustomSelect(s));
}

// Global click listener to close open custom selects on outside click
...
function applyRoleVisibility() {
    ...
    if (crewPlansBtn) crewPlansBtn.style.display = canViewSummary ? '' : 'none';

    document.getElementById('tab-btn-production').style.display = canReport ? '' : 'none';
    document.getElementById('tab-btn-summary').style.display = canViewSummary ? '' : 'none';
    document.getElementById('tab-btn-daily-report').style.display = canViewSummary ? '' : 'none';
    const materialsBtn = document.getElementById('tab-btn-materials');
    if (materialsBtn) materialsBtn.style.display = 'none';
    
    document.getElementById('tab-btn-downtimes').style.display = canDowntime ? '' : 'none';
...
async function loadData() {
    ...
    // Initialize custom selects across the portal
...
function resetReportForm() {
    ...
    const numericIds = [
        ...
        'rep-prev-warehouse-gp', 'rep-prev-first-grade', 'rep-warehouse-gp-total-readonly',
...
async function loadDailyReport() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Renders charts
            renderDailyReportCharts(data.days, data.line_plan_norms);
        }
    } catch(e) {
        console.error(e);
    }
...
function renderDailyReportCharts(days, planNorms) {
    const isDark = document.documentElement.getAttribute('data-theme') !== 'light';
    const textCol = isDark ? '#f8fafc' : '#1e293b';

    // Определение динамических норм плана
    const dayNorm = (planNorms && planNorms.day) ? planNorms.day : 2700;
    const nightNorm = (planNorms && planNorms.night) ? planNorms.night : 3300;
    const avgWeightKg = 19.6;
    const dayTonsNorm = Number(((dayNorm * avgWeightKg) / 1000.0).toFixed(1));
    const nightTonsNorm = Number(((nightNorm * avgWeightKg) / 1000.0).toFixed(1));
    ...
    const sheetsColors = days.map(d => {
        const targetPlan = d.plan_sheets > 0 ? d.plan_sheets : (d.label && d.label.includes('(Д)') ? dayNorm : nightNorm);
        const met = d.fact_sheets >= targetPlan;
        if (met && d.fact_sheets > 0) return '#22c55e'; // Green if plan is met
        return '#ef4444'; // Red if plan is not met
    });

    const tonsColors = days.map(d => {
        const targetTons = d.plan_tons > 0 ? d.plan_tons : (d.label && d.label.includes('(Д)') ? dayTonsNorm : nightTonsNorm);
        const met = d.fact_tons >= targetTons;
            ...
            labels: {
                ...
                generateLabels: function(chart) {
                    return [
                        ...
                        { text: `План День (${dayNorm})`, fillStyle: '#ffc107', strokeStyle: '#ffc107', lineWidth: 2 },
                        { text: `План Ночь (${nightNorm})`, fillStyle: '#8b5cf6', strokeStyle: '#8b5cf6', lineWidth: 2 }
                    ];
                }
            }
        },
    ...
    chartDailySheets = new Chart(ctxSheets, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: `План День (${dayNorm})`,
                    data: Array(days.length).fill(dayNorm),
                ...
                {
                    label: `План Ночь (${nightNorm})`,
                    data: Array(days.length).fill(nightNorm),
    ...
    chartDailyTons = new Chart(ctxTons, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: `План День (${dayTonsNorm} т)`,
                    data: Array(days.length).fill(dayTonsNorm),
                    borderColor: '#ffc107', // Yellow
                    borderWidth: 2,
                    borderDash: [5, 5],
                    fill: false,
                ...
                {
                    label: `План Ночь (${nightTonsNorm} т)`,
                    data: Array(days.length).fill(nightTonsNorm),
                    borderColor: '#8b5cf6', // Purple / Violet
                    borderWidth: 2,
                    borderDash: [5, 5],
                    fill: false,
                    ...
                    labels: {
                        ...
                        generateLabels: function(chart) {
                            return [
                                { text: 'Факт (выполнение)', fillStyle: '#22c55e', strokeStyle: '#22c55e', lineWidth: 1 },
                                { text: 'Факт (невыполнение)', fillStyle: '#ef4444', strokeStyle: '#ef4444', lineWidth: 1 },
                                { text: `План День (${dayTonsNorm} т)`, fillStyle: '#ffc107', strokeStyle: '#ffc107', lineWidth: 2 },
                                { text: `План Ночь (${nightTonsNorm} т)`, fillStyle: '#8b5cf6', strokeStyle: '#8b5cf6', lineWidth: 2 }
                            ];
                        }
                    }
                }
...
const REPORT_FIELDS = [
    ...
    'rep-prev-warehouse-gp', 'rep-prev-first-grade', 'rep-prev-has-defect',
    'prev-def-scratch', 'prev-def-bad-cut', 'prev-def-stick-top', 'prev-def-broken', 'prev-def-fell',
    'prev-def-thickness', 'prev-def-edge',
    ...
    if (saved) {
        try {
            ...
            if (hasData) {
                ...
                if (typeof recalcPrevDefectTotal === 'function') recalcPrevDefectTotal();
                if (typeof recalcWarehouseGPTotal === 'function') recalcWarehouseGPTotal();
...
function createCardHTML(title, subtitle, svgContent, onClickCode, href = null) {
    ...
    const shadowColor = 'rgba(200, 35, 35, 0.25)';
    
    if (href) {
        return `
            <a href="${href}" class="user-card glass-panel" style="text-decoration: none; cursor: pointer; padding: 1.35rem 1rem; text-align: center; border: 1px solid var(--stripe-border); border-radius: 16px; transition: all 0.25s cubic-bezier(0.16, 1, 0.3, 1); display: flex; flex-direction: column; align-items: center; justify-content: center; min-width: 0; box-sizing: border-box; background: #ffffff;">
                <div class="user-avatar-gradient" style="background: ${gradient}; width: 56px; height: 56px; border-radius: 14px; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; box-shadow: 0 4px 12px ${shadowColor}, inset 0 1px 0 rgba(255,255,255,0.25); flex-shrink: 0;">
                    ${svgContent}
                </div>
                <div style="font-weight: 700; font-size: 0.95rem; color: var(--text-primary); margin-bottom: 0.35rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.25; width: 100%; letter-spacing: -0.01em;">${title}</div>
                <div style="font-size: 0.72rem; color: var(--primary-color); font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.2; width: 100%;">${subtitle}</div>
            </a>
        `;
    }
    
    return `
        <div class="user-card glass-panel" onclick="${onClickCode}" style="cursor: pointer; padding: 1.35rem 1rem; text-align: center; border: 1px solid var(--stripe-border); border-radius: 16px; transition: all 0.25s cubic-bezier(0.16, 1, 0.3, 1); display: flex; flex-direction: column; align-items: center; justify-content: center; min-width: 0; box-sizing: border-box; background: #ffffff;">
            <div class="user-avatar-gradient" style="background: ${gradient}; width: 56px; height: 56px; border-radius: 14px; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; box-shadow: 0 4px 12px ${shadowColor}, inset 0 1px 0 rgba(255,255,255,0.25); flex-shrink: 0;">
                ${svgContent}
            </div>
            <div style="font-weight: 700; font-size: 0.95rem; color: var(--text-primary); margin-bottom: 0.35rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.25; width: 100%; letter-spacing: -0.01em;">${title}</div>
